...
CREATE TABLE IF NOT EXISTS luggage_orders (
  ...
  point_usage_status TEXT NOT NULL DEFAULT 'NONE',
  payment_method TEXT,
...
-- Customer account bridge: local luggage profile cache keyed by Account person_id
...
-- Customer points: current balance snapshot keyed by Account person_id
...
-- Customer points: immutable ledger rows for reservations, releases, and earned points
...
CREATE INDEX IF NOT EXISTS idx_luggage_orders_account_recent ON luggage_orders(account_person_id, created_at) WHERE account_person_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_luggage_orders_source_preset ON luggage_orders(source_preset_order_id) WHERE source_preset_order_id IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_luggage_customer_profiles_email ON luggage_customer_profiles(email) WHERE email IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_luggage_point_transactions_person_created ON luggage_customer_point_transactions(account_person_id, created_at);
CREATE INDEX IF NOT EXISTS idx_luggage_point_transactions_order ON luggage_customer_point_transactions(order_id) WHERE order_id IS NOT NULL;
