...
export const NewOrderAlert: FC = () => (
  <div>
    ...
    <script dangerouslySetInnerHTML={{__html: "(function(){var lc=new Date().toISOString();var ac=null;try{var AC=window.AudioContext||window.webkitAudioContext;if(AC)ac=new AC()}catch(e){}function unlockAudio(){if(ac&&ac.state==='suspended'){ac.resume()}document.removeEventListener('click',unlockAudio);document.removeEventListener('touchstart',unlockAudio)}document.addEventListener('click',unlockAudio);document.addEventListener('touchstart',unlockAudio);function beep(){if(!ac)return;try{if(ac.state==='suspended')ac.resume();var o=ac.createOscillator(),g=ac.createGain();o.connect(g);g.connect(ac.destination);o.frequency.value=880;g.gain.value=0.5;o.start();g.gain.exponentialRampToValueAtTime(0.01,ac.currentTime+0.8);o.stop(ac.currentTime+0.8);setTimeout(function(){var o2=ac.createOscillator(),g2=ac.createGain();o2.connect(g2);g2.connect(ac.destination);o2.frequency.value=1100;g2.gain.value=0.5;o2.start();g2.gain.exponentialRampToValueAtTime(0.01,ac.currentTime+0.6);o2.stop(ac.currentTime+0.6)},300)}catch(e){}}setInterval(function(){fetch('/staff/api/orders/new?since='+encodeURIComponent(lc)).then(function(r){return r.json()}).then(function(d){if(d.count>0){beep();lc=new Date().toISOString();if(location.pathname==='/staff/dashboard'){location.reload()}else{var b=document.getElementById('new-order-badge');if(b){b.style.display='block';b.textContent='\\uc0c8 \\uc811\\uc218 '+d.count+'\\uac74';b.onclick=function(){location.href='/staff/dashboard'}}if(document.title.indexOf('\\ud83d\\udd14')===-1)document.title='\\ud83d\\udd14 '+document.title}}}).catch(function(){})},3000)})()"}} />
  </div>
);
