import type { FC } from "hono/jsx";

function getStaffNavLinks(role: string) {
  const primaryLinks = [
    { href: "/staff/dashboard", label: "짐보관 신청" },
    { href: "/staff/cash-closing", label: "정산마감" },
  ];
  const editorLinks = [
    { href: "/staff/admin/sales", label: "매출관리" },
    { href: "/staff/admin/completion-message", label: "완료메시지" },
  ];
  const secondaryLinks = [
    { href: "/staff/handover", label: "인수인계" },
    { href: "/staff/experience", label: "체험단" },
    { href: "/staff/lost-found", label: "분실물" },
    { href: "/staff/schedule", label: "스케줄" },
    { href: "/staff/bug-report", label: "버그신고" },
  ];
  const adminOnlyLinks = [
    { href: "/staff/admin/staff-accounts", label: "계정관리" },
    { href: "/staff/admin/customers", label: "고객목록" },
    { href: "/staff/admin/activity-logs", label: "활동로그" },
  ];

  return [
    ...primaryLinks,
    ...(role === "admin" || role === "editor" ? editorLinks : []),
    ...secondaryLinks,
    ...(role === "admin" ? adminOnlyLinks : []),
  ];
}

export const StaffTopbar: FC<{ staff: { display_name: string | null; username: string | null; role: string }; active?: string }> = ({ staff, active }) => {
  const allLinks = getStaffNavLinks(staff.role);
  return (
    <header class="topbar">
      <div class="topbar-inner">
        <a class="brand" href="/staff/dashboard">
          <img class="brand-logo-horizontal" src="/static/logo-horizontal.png" alt="Flying Japan" height="28" />
        </a>
        <nav class="topbar-menu">
          {allLinks.map(l => <a class={`topbar-menu-link${l.href === active ? " is-active" : ""}`} href={l.href}>{l.label}{l.href === "/staff/handover" && <span id="handover-badge" style="display:none;margin-left:4px;font-size:9px;background:#dc2626;color:#fff;padding:1px 5px;border-radius:8px;font-weight:700;vertical-align:middle"></span>}</a>)}
        </nav>
        <nav class="pill-nav">
          <span class="pill-user">{staff.display_name || staff.username}</span>
          <form method="post" action="/staff/logout" style="display:inline">
            <button type="submit" class="pill-link pill-link-btn">로그아웃</button>
          </form>
        </nav>
      </div>
    </header>
  );
};

export const NewOrderAlert: FC = () => (
  <div>
    <div id="new-order-badge" style="display:none;position:fixed;top:12px;left:50%;transform:translateX(-50%);background:#2383e2;color:#fff;padding:12px 28px;border-radius:16px;font-size:15px;font-weight:700;z-index:9999;cursor:pointer;box-shadow:0 4px 20px rgba(0,0,0,0.15);animation:slideDown .3s ease"></div>
    <style dangerouslySetInnerHTML={{__html: "@keyframes slideDown{from{opacity:0;transform:translateX(-50%) translateY(-20px)}to{opacity:1;transform:translateX(-50%) translateY(0)}}"}} />
    <script dangerouslySetInnerHTML={{__html: "(function(){var lc=new Date().toISOString();var nc=0;var ac=null;try{var AC=window.AudioContext||window.webkitAudioContext;if(AC)ac=new AC()}catch(e){}function unlockAudio(){if(ac&&ac.state==='suspended'){ac.resume()}document.removeEventListener('click',unlockAudio);document.removeEventListener('touchstart',unlockAudio)}document.addEventListener('click',unlockAudio);document.addEventListener('touchstart',unlockAudio);function beep(){if(!ac)return;try{if(ac.state==='suspended')ac.resume();var t=ac.currentTime;function play(f,s,d,v,type){var o=ac.createOscillator(),g=ac.createGain(),fl=ac.createBiquadFilter();o.type=type||'sine';fl.type='lowpass';fl.frequency.value=4000;o.connect(fl);fl.connect(g);g.connect(ac.destination);o.frequency.setValueAtTime(f,t+s);g.gain.setValueAtTime(0,t+s);g.gain.linearRampToValueAtTime(v,t+s+0.01);g.gain.setValueAtTime(v,t+s+0.05);g.gain.exponentialRampToValueAtTime(0.001,t+s+d);o.start(t+s);o.stop(t+s+d)}play(1396.9,0,0.12,0.25,'sine');play(1396.9,0,0.12,0.08,'triangle');play(1864.7,0.12,0.18,0.3,'sine');play(1864.7,0.12,0.18,0.1,'triangle')}catch(e){}}function showBadge(count){nc+=count;var b=document.getElementById('new-order-badge');if(b){b.style.display='block';b.innerHTML='\\ud83d\\udce6 \\uc0c8 \\uc811\\uc218 <strong style=\"font-size:20px;margin:0 4px\">'+nc+'</strong>\\uac74';b.onclick=function(){location.href='/staff/dashboard'}}if(document.title.indexOf('\\ud83d\\udd14')===-1)document.title='\\ud83d\\udd14 '+document.title}setInterval(function(){fetch('/staff/api/orders/new?since='+encodeURIComponent(lc)).then(function(r){return r.json()}).then(function(d){if(d.count>0){beep();lc=new Date().toISOString();var canReload=typeof window.canReloadLuggageDashboard!=='function'||window.canReloadLuggageDashboard();if(location.pathname==='/staff/dashboard'&&canReload){location.reload()}else{showBadge(d.count)}}}).catch(function(){})},3000)})()"}} />
    <script dangerouslySetInnerHTML={{__html: "(function(){function ub(){fetch('/staff/api/handover/unread').then(function(r){return r.json()}).then(function(d){var b=document.getElementById('handover-badge');if(b){if(d.count>0){b.style.display='inline';b.textContent=d.count}else{b.style.display='none'}}}).catch(function(){})}ub();setInterval(ub,30000)})()"}} />
  </div>
);

export const StaffMenu: FC<{ active: string; role: string }> = ({ active, role }) => {
  const links = getStaffNavLinks(role);
  return (
    <nav class="staff-menu" aria-label="직원 메뉴">
      {links.map(l => <a class={`staff-menu-link${l.href === active ? " is-active" : ""}`} href={l.href}>{l.label}</a>)}
    </nav>
  );
};
