...
export const NewOrderAlert: FC = () => (
  <div>
    ...
    <script dangerouslySetInnerHTML={{__html: "(function(){var lc=new Date().toISOString();var ac=null;try{var AC=window.AudioContext||window.webkitAudioContext;if(AC)ac=new AC()}catch(e){}function unlockAudio(){if(ac&&ac.state==='suspended'){ac.resume()}document.removeEventListener('click',unlockAudio);document.removeEventListener('touchstart',unlockAudio)}document.addEventListener('click',unlockAudio);document.addEventListener('touchstart',unlockAudio);function beep(){if(!ac)return;try{if(ac.state==='suspended')ac.resume();var t=ac.currentTime;function tone(f,s,d,v){var o=ac.createOscillator(),g=ac.createGain();o.type='sine';o.connect(g);g.connect(ac.destination);o.frequency.value=f;g.gain.setValueAtTime(0,t+s);g.gain.linearRampToValueAtTime(v,t+s+0.02);g.gain.exponentialRampToValueAtTime(0.001,t+s+d);o.start(t+s);o.stop(t+s+d)}tone(523.25,0,0.25,0.35);tone(659.25,0.3,0.25,0.35);tone(783.99,0.6,0.4,0.4)}catch(e){}}setInterval(function(){fetch('/staff/api/orders/new?since='+encodeURIComponent(lc)).then(function(r){return r.json()}).then(function(d){if(d.count>0){beep();lc=new Date().toISOString();if(location.pathname==='/staff/dashboard'){location.reload()}else{var b=document.getElementById('new-order-badge');if(b){b.style.display='block';b.textContent='\\uc0c8 \\uc811\\uc218 '+d.count+'\\uac74';b.onclick=function(){location.href='/staff/dashboard'}}if(document.title.indexOf('\\ud83d\\udd14')===-1)document.title='\\ud83d\\udd14 '+document.title}}}).catch(function(){})},3000)})()"}} />
  </div>
);
